...
REPORT={'release':wr.RELEASE,'checks':[],'limits':['Chromium simulated viewports, not physical handsets.','Arabic native-speaker proofreading not performed.','Contact form validation and transport mock tested; no real inquiry or receipt test.','Business content reviewed by the author against owner-approved narrative; no independent reader study.']}
...
def static_checks():
 ...
 for lang in wr.LANGS:
  ...
  check(f'{lang}: approved brand slogan preserved',h.select_one('#brand-slogan').get_text()=='Elegant Engineering. Intelligent Operations. Flowmatic.')
...
def browser_checks(base,baseline=None,capture_assets=False):
 ...
 with sync_playwright() as p:
  ...
  for lang in wr.LANGS:
   ...
   go(f'/{lang}/')
   page.locator('.hero-actions a.primary').click();check(f'{lang}: company CTA leads to product explanation',urlsplit(page.url).fragment=='products')
   ...
   for slug in ['home']+list(site.PRODUCTS)+list(site.FACTORY_OS_PAGES):
    ...
    for w,h in SIZES:
     ...
     if slug=='home':
      ...
      cta=page.locator('.hero-actions a.primary');check(f'{lang}: product CTA available without animation prerequisite {w}x{h}',cta.is_visible() and cta.is_enabled())
   ...
   page.set_viewport_size({'width':390,'height':844});go(f'/{lang}/');check(f'{lang}: cross-workflow animation within principle explanation',page.locator('#workflow [data-field-story]').count()==1)
   page.locator('[data-field-story]').scroll_into_view_if_needed();x=page.locator('[data-field-story]').screenshot();page.wait_for_timeout(650);y=page.locator('[data-field-story]').screenshot();check(f'{lang}: field-principle animation advances',x!=y)
   go(f'/{lang}/operations-intelligence/');check(f'{lang}: purchasing is not misrepresented by generic field animation',page.locator('[data-field-story]').count()==0)
