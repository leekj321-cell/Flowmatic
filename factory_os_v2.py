...
HOME_OVERRIDES = {
    "ko": {
        ...
        "description": "Flowmatic은 기존 설비·MES·엑셀을 교체하지 않고, 흩어진 현장 신호를 판단·우선순위·작업·완료 확인으로 연결하는 제조 운영지능 소프트웨어입니다.",
        ...
        "body": "기존 설비·MES·엑셀을 교체하지 않고, 흩어진 현장 신호를 판단·우선순위·작업·완료 확인으로 연결합니다.",
        "support": "스마트팩토리가 데이터를 수집하고 보여준다면, Flowmatic은 무엇을 먼저 처리해야 하는지 정하고 끝날 때까지 추적합니다.",
        "primary": "Flowmatic이 바꾸는 업무",
        "secondary": "4개 지능축 보기",
    ...
    "en": {
        "title": "Flowmatic | Manufacturing Intelligence Platform",
        "description": "Flowmatic is manufacturing operating-intelligence software that works above existing equipment, MES, and spreadsheets—turning scattered signals into priorities, owned work, and verified completion.",
        "eyebrow": "Manufacturing Intelligence Platform",
        # Brand lock: do not override the fixed first-fold Flowmatic slogan.
        "body": "Work above existing equipment, MES, and spreadsheets to turn scattered field signals into priorities, owned work, and verified completion.",
        "support": "Smart-factory systems collect and display data. Flowmatic decides what needs attention next and tracks the response to closure.",
        "primary": "See what changes",
        "secondary": "Explore 4 domains",
    ...
    "ar": {
        ...
        "description": "Flowmatic هو برنامج ذكاء تشغيلي للتصنيع يعمل فوق المعدات القائمة وMES وجداول البيانات، ويحوّل الإشارات المتفرقة إلى أولويات وعمل مسؤول وإنجاز موثّق.",
        "eyebrow": "Manufacturing Intelligence Platform",
        # Brand lock: do not override the fixed first-fold Flowmatic slogan.
        "body": "نعمل فوق المعدات القائمة وMES وجداول البيانات لنحوّل إشارات الميدان المتفرقة إلى أولويات وعمل مسؤول وإنجاز موثّق.",
        "support": "تجمع أنظمة المصنع الذكي البيانات وتعرضها. يحدد Flowmatic ما يحتاج إلى الاهتمام تالياً ويتابع الاستجابة حتى الإغلاق.",
        "primary": "شاهد ما يتغير",
        "secondary": "استكشف مجالات الذكاء الأربعة",
...
PLATFORM = {
    "title": l(
        "하나의 제조 Context.|재사용하는 Engine과 Module.",
        "One manufacturing context.|Reusable engines and modules.",
        "سياق تصنيع واحد.|محركات ووحدات قابلة لإعادة الاستخدام.",
    ),
    "body": l(
        "전문 Engine이 같은 Manufacturing Context를 읽고 Module로 조합됩니다. Event Bus·Audit·Adapter는 모든 Module이 공유하는 경량 런타임입니다.",
        "Domain engines read the same Manufacturing Context and compose into modules. Event Bus, Audit, and adapters remain lightweight shared runtime infrastructure.",
        "تقرأ المحركات المتخصصة سياق التصنيع نفسه وتتجمع في وحدات، بينما يبقى Event Bus وAudit والموصلات بنية تشغيل مشتركة خفيفة.",
    ),
    ...
    "layers": [
        ("Manufacturing Context", l("Shared meaning", "Shared meaning", "معنى مشترك"), l("공장·설비·제품·공정·공구·Feature·시간의 공통 기준을 유지합니다.", "Maintain shared meaning for factory, equipment, product, process, tool, feature, and time.", "الحفاظ على معنى مشترك للمصنع والمعدة والمنتج والعملية والأداة والميزة والزمن.")),
        ("Engine Pool", l("Reusable domain capability", "Reusable domain capability", "قدرات متخصصة قابلة لإعادة الاستخدام"), l("NC Semantic·Geometry·Machine State·Quality Analysis 등 필요한 제조 의미를 선택합니다.", "Select only the manufacturing meaning required, such as NC Semantic, Geometry, Machine State, or Quality Analysis.", "اختيار معنى التصنيع المطلوب فقط مثل NC Semantic وGeometry وMachine State وQuality Analysis.")),
        ("Module Pool", l("Operational composition", "Operational composition", "تركيب تشغيلي"), l("Engine을 Viewer·TMS·Root Cause 같은 현장 업무 Module로 조합합니다.", "Compose engines into field modules such as Viewer, TMS, and Root Cause.", "تركيب المحركات في وحدات ميدانية مثل Viewer وTMS وRoot Cause.")),
        ("Solution Profiles", l("Customer-specific composition", "Customer-specific composition", "تركيب خاص بالعميل"), l("필요한 Module만 고객 문제별로 조합합니다. Control Tower도 고정 상위 계층이 아닌 조합 대상 Module입니다.", "Compose only the modules each customer problem needs. Control Tower is a composition-target module, not a fixed top layer.", "تُركّب الوحدات التي تحتاجها مشكلة العميل فقط، وControl Tower وحدة مستهدفة للتركيب وليست طبقة علوية ثابتة.")),
    ],
}

EVIDENCE = [
    (l("가공 레시피 저장·조립", "Recipe persistence and NC assembly", "حفظ وصفة التشغيل وتجميع NC"), l("소스 기준 검증 완료", "Source-level validated", "متحقق على مستوى المصدر"), l("제품·공정·작업 단위의 가공 레시피를 저장하고, 단위 가공 코드를 지정한 순서대로 조립해 최종 NC를 구성합니다.", "Manufacturing Recipes persist product, process, and operation scope, then assemble stored code blocks in a controlled order into final NC.", "تحفظ وصفة التشغيل نطاق المنتج والعملية والتشغيل، ثم تجمع كتل الكود المحفوظة بترتيب مضبوط لتكوين NC النهائي."), "machining-intelligence"),
    (l("NC 조립 전 안전 경계 검사", "Safety checks before NC assembly", "فحوص السلامة قبل تجميع NC"), l("소스 기준 검증 완료", "Source-level validated", "متحقق على مستوى المصدر"), l("좌표계·공구·사이클·안전 높이의 상태를 확인하고, 불확실하거나 위험한 경우 조립을 중단해 검토 대상으로 남깁니다.", "Machine-state boundaries are checked before NC assembly. Unknown or unsafe states stop assembly and require review.", "تُفحص حدود حالة الماكينة قبل تجميع NC، وتوقف الحالات غير المعروفة أو غير الآمنة التجميع للمراجعة."), "machining-intelligence"),
    (l("기존 G-code 해석과 현장 동기화", "Legacy G-code context and field synchronization", "سياق G-code القائم والمزامنة الميدانية"), l("개발 중 · PoC 범위", "Active development / PoC", "تطوير نشط / نطاق PoC"), l("관리되지 않은 G-code의 공정 문맥 복원, 제조 메타데이터 기록, USB 기반 버전 동기화와 CNC·CMM 연동을 개발하고 있습니다.", "Development covers unmanaged G-code context recovery, managed manufacturing metadata, USB revision sync, and CNC/CMM integration.", "يشمل التطوير استعادة سياق G-code غير المدار، وبيانات التصنيع الوصفية، ومزامنة المراجعات عبر USB، وتكامل CNC/CMM."), "machining-intelligence"),
    (l("손실 기준 품질 개선", "Quality improvement ranked by loss", "تحسين الجودة حسب الخسارة"), l("작동 프로토타입", "Working prototype", "نموذج عامل"), l("불량을 손실과 개선 우선순위로 연결하고, 담당 업무와 조치 효과, 재발 여부까지 이어서 확인합니다.", "Defects are connected to loss-ranked priorities, assigned work, verified effects, and recurrence checks.", "تُربط العيوب بأولويات مرتبة حسب الخسارة، ثم بالعمل المكلف والتحقق من الأثر ومتابعة التكرار."), "quality"),
    (l("요청·사용량·비용 분석", "Requests, usage, and cost analysis", "تحليل الطلبات والاستخدام والتكلفة"), l("기능형 MVP · 내부 검증", "Functional MVP / internal validation", "MVP وظيفي / تحقق داخلي"), l("메일·메신저 요청을 품목과 수량으로 정리하고, 생산량 대비 자원 사용과 비용의 이상 징후를 찾습니다.", "Requests are structured into items and quantities, while resource use and cost are compared with production to surface anomalies.", "تُنظم الطلبات إلى أصناف وكميات، ويُقارن استخدام الموارد والتكلفة بالإنتاج لإظهار مؤشرات الشذوذ."), "operations-intelligence"),
    (l("자재 요청부터 투입 확인까지", "From material request to input confirmation", "من طلب المواد إلى تأكيد الإدخال"), l("프로토타입 통합", "Prototype integration", "تكامل نموذج أولي"), l("자재 요청의 우선순위를 정하고 실행자를 배정한 뒤, 운반과 실제 투입 완료까지 상태를 연결합니다.", "Material requests are prioritized and assigned, then tracked through dispatch, arrival, and actual input confirmation.", "تُرتب طلبات المواد وتُسند إلى المنفذ، ثم تُتابع عبر الإرسال والوصول وتأكيد الإدخال الفعلي."), "logistics-intelligence"),
]
...
ROADMAP = [
    ...
    ("03", "Shared Context + Engine Pool", l("공통 제조 문맥과 재사용 가능한 전문 Engine 계약을 구축합니다.", "Establish shared manufacturing context and reusable domain-engine contracts.", "إنشاء سياق تصنيع مشترك وعقود محركات متخصصة قابلة لإعادة الاستخدام.")),
    ("04", "Module + Solution Composition", l("Engine을 현장 Module로 조합하고 고객 문제별 Solution Profile을 검증합니다.", "Compose engines into operational modules and validate customer-specific solution profiles.", "تركيب المحركات في وحدات تشغيل والتحقق من ملفات حلول خاصة بمشكلة العميل.")),
...
PAGES = {
    "operations-intelligence": {
        ...
        "hero": l("현장 신호와 요청을 Event로 바꾸고,|담당·조치·완료까지 연결합니다.", "Turn field signals and requests into events.|Connect ownership, action, and completion.", "حوّل إشارات الميدان وطلباته إلى أحداث.|واربط المسؤول والإجراء والإنجاز."),
        "body": l("설비 상태·현장 요청·자원 사용량을 같은 운영 문맥으로 기록하고 우선순위와 다음 행동을 연결합니다. 조달 요청 구조화와 비용·소비 이상 탐지는 현재 구현 범위에서 확인할 수 있습니다.", "Record machine state, field requests, and resource use in one operating context, then connect priority and next action. The current build includes procurement-request structuring and cost or consumption anomaly detection.", "سجّل حالة المعدات وطلبات الميدان واستخدام الموارد في سياق تشغيلي واحد، ثم اربط الأولوية بالإجراء التالي. ويشمل البناء الحالي تنظيم طلبات الشراء واكتشاف شذوذ التكلفة أو الاستهلاك."),
        "status": l("Functional MVP / internal validation", "Functional MVP / internal validation", "MVP وظيفي / تحقق داخلي"),
    ...
    "platform": {
        "label": "Manufacturing Intelligence Platform",
...
BEFORE_AFTER = {
    "ko": {
        "title": "기존 시스템은 기록하고 보여줍니다.|Flowmatic은 판단과 후속 업무를 끝까지 연결합니다.",
        "body": "MES·SCADA·ERP·CNC·검사장비의 책임은 그대로 둡니다. Flowmatic은 시스템 사이에서 사람이 반복하던 해석·우선순위·전달·확인을 하나의 운영 흐름으로 연결합니다.",
        "before": "현장 데이터와 설비를 관리한다",
        "before_items": ["설비 상태와 생산실적 수집", "기준정보와 이력 저장", "현황·알람·지표 표시", "인증된 설비와 안전제어 유지"],
        "after": "무엇을 먼저 처리할지 정하고 종결한다",
    },
    "en": {
        "title": "Existing systems record and display.|Flowmatic connects decisions to completed work.",
        "body": "MES, SCADA, ERP, CNC, and inspection equipment keep their existing responsibilities. Flowmatic connects the interpretation, prioritization, handoff, and confirmation people repeat between them into one operating flow.",
        "before": "Manage field data and equipment",
        "before_items": ["Collect machine state and production results", "Store master data and history", "Display status, alarms, and metrics", "Retain certified equipment and safety control"],
        "after": "Decide what comes first and close the loop",
    },
    "ar": {
        "title": "تسجّل الأنظمة القائمة وتعرض.|ويربط Flowmatic القرار بالعمل حتى اكتماله.",
        "body": "تحتفظ MES وSCADA وERP وCNC ومعدات الفحص بمسؤولياتها القائمة. ويربط Flowmatic التفسير وتحديد الأولوية والتسليم والتأكيد التي يكررها الأشخاص بينها في تدفق تشغيلي واحد.",
        "before": "إدارة بيانات الميدان والمعدات",
        "before_items": ["جمع حالة المعدات ونتائج الإنتاج", "حفظ البيانات الأساسية والسجل", "عرض الحالة والتنبيهات والمؤشرات", "الإبقاء على المعدات المعتمدة والتحكم الآمن"],
        "after": "تحديد ما يأتي أولاً وإغلاق الحلقة",
    },
